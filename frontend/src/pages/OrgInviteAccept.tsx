...
import { acceptOrgInvite, ApiError, declineOrgInvite, getOrgInviteByToken, type OrgInviteDetails } from '../lib/api';
...
export default function OrgInviteAccept() {
  ...
  const handleAccept = async () => {
    ...
    setError(null);
    try {
      const { orgId } = await acceptOrgInvite(token);
      refresh();
      selectOrg(orgId);
      navigate(`/orgs/${orgId}`);
    } catch (err) {
    ...
    <div className="new-project-page">
      ...
      <main className="new-project-main">
        <div className="new-project-breadcrumb">
          ...
          <span className="new-project-breadcrumb-current">Organization invite</span>
        </div>
        <div className="new-project-divider" />

        <div className="new-project-eyebrow">Organization invitation</div>
        ...
        {needsLogin ? (
          ...
          <>
            <h1 className="new-project-title">Join {invite.orgName}</h1>
